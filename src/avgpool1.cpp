...
int main(int argc, char *argv[])
{
    ...
    void *argtable[] = {a_fi, a_lk, a_str, a_pad, a_pm, a_cm, a_fo, a_help, a_end};
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::avgpool1_d(Y,X,N,Li,Lk,pad,str,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::avgpool1_z(Y,X,N,Li,Lk,pad,str,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }

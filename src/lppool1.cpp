...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "Use -w (--pow) to give the power exponent in X.^pow [default=2.0 -> RMS]\n";
    descr += "The exponent is constrained here to be positive.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ lppool1 -k5 X -o Y \n";
    descr += "$ lppool1 -w3 -k9 -s2 -p10 -m'reflect' X > Y \n";
    descr += "$ cat X | lppool1 -w1.5 -k12 -c -s5 -p10 -m'c' - > Y \n";
    ...
    struct arg_dbl   *a_pw = arg_dbln("w","pow","<dbl>",0,1,"power in X.^pow [default=2.0]");
    struct arg_file  *a_fo = arg_filen("o","ofile","<file>",0,O,"output file (Y)");
    struct arg_lit *a_help = arg_litn("h","help",0,1,"display this help and exit");
    struct arg_end  *a_end = arg_end(5);
    void *argtable[] = {a_fi, a_lk, a_str, a_pad, a_pm, a_cm, a_pw, a_fo, a_help, a_end};
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::lppool1_s(Y,X,N,Li,Lk,pad,str,ceil_mode,pm,(float)pw))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::lppool1_d(Y,X,N,Li,Lk,pad,str,ceil_mode,pm,(double)pw))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::lppool1_c(Y,X,N,Li,Lk,pad,str,ceil_mode,pm,(float)pw))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::lppool1_z(Y,X,N,Li,Lk,pad,str,ceil_mode,pm,(double)pw))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }

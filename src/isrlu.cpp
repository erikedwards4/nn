...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets inverse square-root linear unit (ISRLU) for each element of X.\n";
    descr += "For each element: y = x / sqrt(1+alpha*x),  if x<0.\n";
    descr += "                  y = x,                    if x>=0.\n";
    descr += "\n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::isrlu_inplace_s(X,int(i1.N()),float(alpha)))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::isrlu_inplace_d(X,int(i1.N()),double(alpha)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

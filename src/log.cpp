...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets natural logarithm (ln) of each element of X.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ log X -o Y \n";
    descr += "$ log X > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::log_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::log_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (openn::log_inplace_c(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (openn::log_inplace_z(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

...
int main(int argc, char *argv[])
{
    ...
    descr += "Does middle stage of Hopfield model for each row or col of X.\n";
    ...
    descr += "\n";
    descr += "For dim=0, Y[n,t] = a[n]*Y[n,t-1] + b[n]*(X[n,t]-alpha[n]*Y[n,t-1]). \n";
    descr += "with sizes X:  N x T \n";
    descr += "           Y:  N x T \n";
    descr += "\n";
    descr += "For dim=1, Y[t,n] = a[n]*Y[t-1,n] + b[n]*(X[t,n]-alpha[n]*Y[t-1,n]). \n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ hopfield X taus alphas -o Y \n";
    descr += "$ hopfield X taus alphas > Y \n";
    descr += "$ cat X | hopfield - taus alphas > Y \n";


    //Argtable
    int nerrs;
    struct arg_file  *a_fi = arg_filen(nullptr,nullptr,"<file>",I-1,I,"input files (X,taus,alphas)");
    ...
    else if (i1.T==2)
    {
        ...
        if (i2.N()==1u) { for (size_t n=1u; n<N; ++n) { taus[n] = taus[0]; } }
        if (i3.N()==1u) { for (size_t n=1u; n<N; ++n) { alphas[n] = alphas[0]; } }
        //if (codee::hopfield_d(Y,X,taus,N,T,i1.iscolmajor(),dim,double(fs)))
        if (codee::hopfield_inplace_d(X,taus,alphas,N,T,i1.iscolmajor(),dim,double(fs)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; } 
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        delete[] X; delete[] taus; delete[] alphas;
    }
    else if (i1.T==102u)
    {
        double *X, *taus, *alphas;
        ...
        if (i2.N()==1u) { for (size_t n=1u; n<N; ++n) { taus[n] = taus[0]; } }
        if (i3.N()==1u) { for (size_t n=1u; n<N; ++n) { alphas[n] = alphas[0]; } }
        if (codee::hopfield_inplace_z(X,taus,alphas,N,T,i1.iscolmajor(),dim,double(fs)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; } 
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

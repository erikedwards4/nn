...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets inverse hyperbolic sine (asinh) of each element of X.\n";
    descr += "For each element: y = asinh(x).\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ asinh X -o Y \n";
    descr += "$ asinh X > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::asinh_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::asinh_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

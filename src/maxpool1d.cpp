...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "1d max-pooling, with full opts of conv1d (e.g., dilation).\n";
    descr += "\n";
    ...
    descr += "Use pad_mode 'no_count_pad' or 'n' to emulate usual MaxPool1d behavior.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ maxpool1d -k5 X -o Y \n";
    descr += "$ maxpool1d -k9 -s2 -p10 -m'reflect' X > Y \n";
    descr += "$ cat X | maxpool1d -k12 -i2 -c -s5 -p10 -m'c' - > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::maxpool1d_s(Y,X,N,Li,Lk,pad,str,dil,ceil_mode,pm))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::maxpool1d_d(Y,X,N,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::maxpool1d_c(Y,X,N,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::maxpool1d_z(Y,X,N,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }

...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "1d cross-correlation to mostly emulate PyTorch Conv1d.\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ conv1d.cblas X K B -o Y \n";
    descr += "$ conv1d.cblas -s2 -i2 -m'reflect' X K B > Y \n";
    descr += "$ cat X | conv1d.cblas -c -s5 -p10 -m'c' - K B > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::conv1d_cblas_s(Y,X1,X2,X3,Ni,No,Li,Lk,pad,str,dil,ceil_mode,pm))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::conv1d_cblas_d(Y,X1,X2,X3,Ni,No,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::conv1d_cblas_c(Y,X1,X2,X3,Ni,No,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::conv1d_cblas_z(Y,X1,X2,X3,Ni,No,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }

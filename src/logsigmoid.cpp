...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets logsigmoid function (log of sigmoid) of each element of X.\n";
    descr += "For each element: y = log(1/(1+exp(-x))) = -log(1+exp(-x)).\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ logsigmoid X -o Y \n";
    descr += "$ logsigmoid X > Y \n";
    descr += "$ cat X | logsigmoid > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::logsigmoid_inplace_s(X,i1.N()))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::logsigmoid_inplace_d(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

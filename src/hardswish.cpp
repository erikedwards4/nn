...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets the Hardswish function [Howard et al. 2019] of each element of X.\n";
    descr += "For each element: y = 0,         if x<=-3.\n";
    descr += "                  y = x,         if x>=+3.\n";
    descr += "                  y = x*(x+3)/6, otherwise.\n";
    descr += "\n";
    descr += "Use -b (--beta) to specify the beta parameter [default=1].\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ hardswish X -o Y \n";
    descr += "$ hardswish X > Y \n";
    descr += "$ cat X | hardswish > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::hardswish_inplace_s(X,i1.N()))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::hardswish_inplace_d(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

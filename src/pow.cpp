...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ pow X -p0.1 -o Y \n";
    descr += "$ pow X -p0.1 > Y \n";
    descr += "$ cat X | pow -p0.1 > Y \n";


    //Argtable
    int nerrs;
    struct arg_file  *a_fi = arg_filen(nullptr,nullptr,"<file>",I-1,I,"input file (X)");
    struct arg_dbl    *a_p = arg_dbln("p","pow","<dbl>",0,1,"power [default=1.0]");
    struct arg_file  *a_fo = arg_filen("o","ofile","<file>",0,O,"output file (Y)");
    struct arg_lit *a_help = arg_litn("h","help",0,1,"display this help and exit");
    struct arg_end  *a_end = arg_end(5);
    void *argtable[] = {a_fi, a_p, a_fo, a_help, a_end};
    ...
    if (i1.T==1)
    {
        ...
        if (openn::pow_inplace_s(X,int(i1.N()),float(p)))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::pow_inplace_d(X,int(i1.N()),double(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (openn::pow_inplace_c(X,int(i1.N()),float(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (openn::pow_inplace_z(X,int(i1.N()),double(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

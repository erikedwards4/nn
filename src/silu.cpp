...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets sigmoid-weighted linear unit (SiLU) [Elfwing et al. 2017] of each element of X.\n";
    descr += "For each element: y = x/(1+exp(-x)).\n";
    descr += "\n";
    descr += "This is the same as the Swish with beta=1.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ silu X -o Y \n";
    descr += "$ silu X > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::silu_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::silu_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

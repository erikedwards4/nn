...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "1d convolution, same as conv1d.cblas, but no dilation.\n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::conv1_cblas_s(Y,X,K,B,Ni,No,Nb,Li,Lk,pad,str,ceil_mode,pm))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::conv1_cblas_d(Y,X,K,B,Ni,No,Nb,Li,Lk,pad,str,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::conv1_cblas_c(Y,X,K,B,Ni,No,Nb,Li,Lk,pad,str,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::conv1_cblas_z(Y,X,K,B,Ni,No,Nb,Li,Lk,pad,str,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }

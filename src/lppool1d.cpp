...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "1d Lp-pooling, with full opts of conv1d (e.g., dilation).\n";
    descr += "This does mean(X.^pw)^(1/pw).\n";
    descr += "This differs slightly from PyTorch, because a mean\n";
    descr += "rather than a sum is taken, but the output is proportional.\n";
    descr += "Thus, for pw==1, this is same as avgpool1d.\n";
    descr += "And for pw==2, this a root-mean-square.\n";
    descr += "\n";
    ...
    descr += "Use pad_mode 'no_count_pad' or 'n' to emulate usual LpPool1d behavior.\n";
    descr += "\n";
    descr += "Use -w (--pow) to give the power exponent in X.^pow [default=2 -> RMS]\n";
    descr += "The exponent is constrained here to be positive.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ lppool1d -k5 X -o Y \n";
    descr += "$ lppool1d -w3 -k9 -s2 -p10 -m'reflect' X > Y \n";
    descr += "$ cat X | lppool1d -w1.5 -k12 -i2 -c -s5 -p10 -m'c' - > Y \n";
    ...
    struct arg_dbl   *a_pw = arg_dbln("w","pow","<dbl>",0,1,"power in X.^w [default=2]");
    struct arg_file  *a_fo = arg_filen("o","ofile","<file>",0,O,"output file (Y)");
    struct arg_lit *a_help = arg_litn("h","help",0,1,"display this help and exit");
    struct arg_end  *a_end = arg_end(5);
    void *argtable[] = {a_fi, a_lk, a_str, a_dil, a_pad, a_pm, a_cm, a_pw, a_fo, a_help, a_end};
    ...
    //Get pw
    if (a_pw->count==0) { pw = 2.0; }
    else if (a_pw->dval[0]<double(FLT_EPSILON)) { cerr << progstr+": " << __LINE__ << errstr << "pow must be positive" << endl; return 1; }
    else { pw = a_pw->dval[0]; }
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::lppool1d_s(Y,X,N,Li,Lk,pad,str,dil,ceil_mode,pm,(float)pw))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::lppool1d_d(Y,X,N,Li,Lk,pad,str,dil,ceil_mode,pm,(double)pw))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::lppool1d_c(Y,X,N,Li,Lk,pad,str,dil,ceil_mode,pm,(float)pw))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::lppool1d_z(Y,X,N,Li,Lk,pad,str,dil,ceil_mode,pm,(double)pw))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }

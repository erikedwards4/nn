...
int main(int argc, char *argv[])
{
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::conv1d_s(Y,X,K,B,Ni,No,Nb,Li,Lk,pad,str,dil,ceil_mode,pm))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::conv1d_d(Y,X,K,B,Ni,No,Nb,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::conv1d_c(Y,X,K,B,Ni,No,Nb,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::conv1d_z(Y,X,K,B,Ni,No,Nb,Li,Lk,pad,str,dil,ceil_mode,pm))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }

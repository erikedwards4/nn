...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets inverse square-root unit (ISRU) for each element of X.\n";
    descr += "This is called an algebraic sigmoid function by Wikipedia (with alpha=1).\n";
    descr += "For each element: y = x / sqrt(1+alpha*x).\n";
    descr += "\n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::isru_inplace_s(X,int(i1.N()),float(alpha)))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::isru_inplace_d(X,int(i1.N()),double(alpha)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

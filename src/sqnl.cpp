...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets SQNL function [Wuraola & Patel 2018] for each element of X.\n";
    descr += "For each element: y = 1,            if x>=2.\n";
    descr += "                  y = x - 0.25*x^2, if 0<x<2.\n";
    descr += "                  y = x + 0.25*x^2, if 0>x>-2.\n";
    descr += "                  y = -1,           if x<-2.\n";
    descr += "\n";
    descr += "Examples:\n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::sqnl_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::sqnl_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

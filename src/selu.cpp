...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets scaled exponential linear unit (SELU) [Klambauer et al. 2017] of each element of X.\n";
    descr += "For each element: y = lambda*alpha*(exp(x)-1),  if x<0.\n";
    descr += "                  y = lambda*x,                 if x>=0.\n";
    descr += "\n";
    descr += "The parameters are fixed at: alpha=1.67326 and lambda=1.0507.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ selu X -o Y \n";
    descr += "$ selu X > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::selu_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::selu_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

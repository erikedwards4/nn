...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets variance along rows or cols of X.\n";
    descr += "\n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::var_s(Y,X,int(i1.R),int(i1.C),dim,i1.iscolmajor(),b))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::var_d(Y,X,int(i1.R),int(i1.C),dim,i1.iscolmajor(),b))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (openn::var_c(Y,X,int(i1.R),int(i1.C),dim,i1.iscolmajor(),b))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (openn::var_z(Y,X,int(i1.R),int(i1.C),dim,i1.iscolmajor(),b))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }

...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets softsign function (aka ElliotSig) for each element of X.\n";
    descr += "For each element: y = x/(1+|x|).\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ softsign X -o Y \n";
    descr += "$ softsign X > Y \n";
    descr += "$ cat X | softsign > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::softsign_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::softsign_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }

...
int swish_s (float *Y, const float *X, const size_t N, const float beta)
{
    ...
    if (beta==0.0f)
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = 0.5f**X; }
    }
    else if (beta==1.0f)
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = *X/(1.0f+expf(-*X)); }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = *X/(1.0f+expf(-beta**X)); }
    }
...
int swish_d (double *Y, const double *X, const size_t N, const double beta)
{
    ...
    if (beta==0.0)
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = 0.5**X; }
    }
    else if (beta==1.0)
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = *X/(1.0+exp(-*X)); }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = *X/(1.0+exp(-beta**X)); }
    }
...
int swish_inplace_s (float *X, const size_t N, const float beta)
{
    ...
    if (beta==0.0f)
    {
        for (size_t n=0; n<N; ++n, ++X) { *X *= 0.5f; }
    }
    else if (beta==1.0f)
    {
        for (size_t n=0; n<N; ++n, ++X) { *X /= 1.0f + expf(-*X); }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X) { *X /= 1.0f + expf(-beta**X); }
    }
...
int swish_inplace_d (double *X, const size_t N, const double beta)
{
    ...
    if (beta==0.0)
    {
        for (size_t n=0; n<N; ++n, ++X) { *X *= 0.5; }
    }
    else if (beta==1.0)
    {
        for (size_t n=0; n<N; ++n, ++X) { *X /= 1.0 + exp(-*X); }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X) { *X /= 1.0 + exp(-beta**X); }
    }

...
int fir_s (float *Y, const float *X, const float *B, const size_t N, const size_t T, const int L, const char iscolmajor, const size_t dim);
int fir_d (double *Y, const double *X, const double *B, const size_t N, const size_t T, const int L, const char iscolmajor, const size_t dim);
int fir_c (float *Y, const float *X, const float *B, const size_t N, const size_t T, const int L, const char iscolmajor, const size_t dim);
int fir_z (double *Y, const double *X, const double *B, const size_t N, const size_t T, const int L, const char iscolmajor, const size_t dim);
...
int fir_s (float *Y, const float *X, const float *B, const size_t N, const size_t T, const int L, const char iscolmajor, const size_t dim)
{
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            ...
            {
                for (l=0; l<L; l++) { cblas_saxpy(T-l,B[n+l*N],&X[n],(int)N,&Y[n+l*N],(int)N); }
            }
        }
        else
        {
...
int fir_d (double *Y, const double *X, const double *B, const size_t N, const size_t T, const int L, const char iscolmajor, const size_t dim)
{
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            for (size_t n=0; n<N; ++n)
            {
                for (l=0; l<L; l++) { cblas_daxpy(T-l,B[n+l*N],&X[n],(int)N,&Y[n+l*N],(int)N); }
            }
        }
        else
        {
...
int fir_c (float *Y, const float *X, const float *B, const size_t N, const size_t T, const int L, const char iscolmajor, const size_t dim)
{
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            for (size_t n=0; n<N; ++n)
            {
                for (l=0; l<L; l++) { cblas_caxpy(T-l,&B[2*(n+l*N)],&X[2*n],(int)N,&Y[2*(n+l*N)],(int)N); }
            }
        }
        else
        {
...
int fir_z (double *Y, const double *X, const double *B, const size_t N, const size_t T, const int L, const char iscolmajor, const size_t dim)
{
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            for (size_t n=0; n<N; ++n)
            {
                for (l=0; l<L; l++) { cblas_zaxpy(T-l,&B[2*(n+l*N)],&X[2*n],(int)N,&Y[2*(n+l*N)],(int)N); }
            }
        }
        else
        {

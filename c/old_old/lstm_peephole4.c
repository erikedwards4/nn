...
int lstm_peephole4_s (float *Y, const float *Xc, const float *Xi, const float *Xf, const float *Xo, const float *Ui, const float *Uf, const float *Uo, const size_t N, const size_t T, const char iscolmajor, const size_t dim)
{
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            ...
            for (size_t t=1; t<T; ++t)
            {
                ...
                cblas_sgemv(CblasColMajor,CblasNoTrans,(int)N,(int)N,o,Ui,(int)N,(int)C,1,o,I,1);
                cblas_sgemv(CblasColMajor,CblasNoTrans,(int)N,(int)N,o,Uf,(int)N,(int)C,1,o,F,1);
                cblas_sgemv(CblasColMajor,CblasNoTrans,(int)N,(int)N,o,Uo,(int)N,(int)C,1,o,O,1);
                for (size_t n=0; n<N; ++n)
                {
        ...
        else
        {
            ...
            for (size_t t=1; t<T; ++t)
            {
                ...
                cblas_sgemv(CblasRowMajor,CblasNoTrans,(int)N,(int)N,o,Ui,(int)N,(int)C,1,o,I,1);
                cblas_sgemv(CblasRowMajor,CblasNoTrans,(int)N,(int)N,o,Uf,(int)N,(int)C,1,o,F,1);
                cblas_sgemv(CblasRowMajor,CblasNoTrans,(int)N,(int)N,o,Uo,(int)N,(int)C,1,o,O,1);
                for (size_t n=0; n<N; ++n)
                {
                    I[n] = 1.0f / (1.0f+expf(-I[n]));
    ...
    else if (dim==1)
    {
        if (iscolmajor)
        {
            ...
            {
                cblas_scopy((int)N,&Xi[t],(int)T,I,1); cblas_scopy((int)N,&Xf[t],(int)T,F,1); cblas_scopy((int)N,&Xo[t],(int)T,O,1);
                cblas_sgemv(CblasColMajor,CblasTrans,(int)N,(int)N,o,Ui,(int)N,(int)C,1,o,I,1);
                cblas_sgemv(CblasColMajor,CblasTrans,(int)N,(int)N,o,Uf,(int)N,(int)C,1,o,F,1);
                cblas_sgemv(CblasColMajor,CblasTrans,(int)N,(int)N,o,Uo,(int)N,(int)C,1,o,O,1);
                for (size_t n=0; n<N; ++n)
                {
                    I[n] = 1.0f / (1.0f+expf(-I[n]));
                    C[n] = C[n]/(1.0f+expf(-F[n])) + I[n]/(1.0f+expf(-Xc[t+n*T]));
        ...
        else
        {
            ...
            for (size_t t=1; t<T; ++t)
            {
                tN = t*N;
                cblas_scopy((int)N,&Xi[tN],1,I,1); cblas_scopy((int)N,&Xf[tN],1,F,1); cblas_scopy((int)N,&Xo[tN],1,O,1);
                cblas_sgemv(CblasRowMajor,CblasTrans,(int)N,(int)N,o,Ui,(int)N,(int)C,1,o,I,1);
                cblas_sgemv(CblasRowMajor,CblasTrans,(int)N,(int)N,o,Uf,(int)N,(int)C,1,o,F,1);
                cblas_sgemv(CblasRowMajor,CblasTrans,(int)N,(int)N,o,Uo,(int)N,(int)C,1,o,O,1);
                for (size_t n=0; n<N; ++n)
                {
                    I[n] = 1.0f / (1.0f+expf(-I[n]));
                    C[n] = C[n]/(1.0f+expf(-F[n])) + I[n]/(1.0f+expf(-Xc[tN+n]));
...
int lstm_peephole4_d (double *Y, const double *Xc, const double *Xi, const double *Xf, const double *Xo, const double *Ui, const double *Uf, const double *Uo, const size_t N, const size_t T, const char iscolmajor, const size_t dim)
{
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            ...
            for (size_t t=1; t<T; ++t)
            {
                ...
                cblas_dgemv(CblasColMajor,CblasNoTrans,(int)N,(int)N,o,Ui,(int)N,(int)C,1,o,I,1);
                cblas_dgemv(CblasColMajor,CblasNoTrans,(int)N,(int)N,o,Uf,(int)N,(int)C,1,o,F,1);
                cblas_dgemv(CblasColMajor,CblasNoTrans,(int)N,(int)N,o,Uo,(int)N,(int)C,1,o,O,1);
                for (size_t n=0; n<N; ++n)
                {
                    I[n] = 1.0 / (1.0+exp(-I[n]));
        ...
        else
        {
            ...
            for (size_t t=1; t<T; ++t)
            {
                ...
                cblas_dgemv(CblasRowMajor,CblasNoTrans,(int)N,(int)N,o,Ui,(int)N,(int)C,1,o,I,1);
                cblas_dgemv(CblasRowMajor,CblasNoTrans,(int)N,(int)N,o,Uf,(int)N,(int)C,1,o,F,1);
                cblas_dgemv(CblasRowMajor,CblasNoTrans,(int)N,(int)N,o,Uo,(int)N,(int)C,1,o,O,1);
                for (size_t n=0; n<N; ++n)
                {
                    I[n] = 1.0 / (1.0+exp(-I[n]));
    ...
    else if (dim==1)
    {
        if (iscolmajor)
        {
            ...
            {
                cblas_dcopy((int)N,&Xi[t],(int)T,I,1); cblas_dcopy((int)N,&Xf[t],(int)T,F,1); cblas_dcopy((int)N,&Xo[t],(int)T,O,1);
                cblas_dgemv(CblasColMajor,CblasTrans,(int)N,(int)N,o,Ui,(int)N,(int)C,1,o,I,1);
                cblas_dgemv(CblasColMajor,CblasTrans,(int)N,(int)N,o,Uf,(int)N,(int)C,1,o,F,1);
                cblas_dgemv(CblasColMajor,CblasTrans,(int)N,(int)N,o,Uo,(int)N,(int)C,1,o,O,1);
                for (size_t n=0; n<N; ++n)
                {
                    I[n] = 1.0 / (1.0+exp(-I[n]));
                    C[n] = C[n]/(1.0+exp(-F[n])) + I[n]/(1.0+exp(-Xc[t+n*T]));
        ...
        else
        {
            ...
            for (size_t t=1; t<T; ++t)
            {
                tN = t*N;
                cblas_dcopy((int)N,&Xi[tN],1,I,1); cblas_dcopy((int)N,&Xf[tN],1,F,1); cblas_dcopy((int)N,&Xo[tN],1,O,1);
                cblas_dgemv(CblasRowMajor,CblasTrans,(int)N,(int)N,o,Ui,(int)N,(int)C,1,o,I,1);
                cblas_dgemv(CblasRowMajor,CblasTrans,(int)N,(int)N,o,Uf,(int)N,(int)C,1,o,F,1);
                cblas_dgemv(CblasRowMajor,CblasTrans,(int)N,(int)N,o,Uo,(int)N,(int)C,1,o,O,1);
                for (size_t n=0; n<N; ++n)
                {
                    I[n] = 1.0 / (1.0+exp(-I[n]));
                    C[n] = C[n]/(1.0+exp(-F[n])) + I[n]/(1.0+exp(-Xc[tN+n]));

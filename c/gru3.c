...
int gru3_s (float *Y, const float *X, const float *Xr, const float *Xz, const float *U, const float *Ur, const float *Uz, const int N, const int T, const int dim, const char iscolmajor);
int gru3_d (double *Y, const double *X, const double *Xr, const double *Xz, const double *U, const double *Ur, const double *Uz, const int N, const int T, const int dim, const char iscolmajor);

int gru3_inplace_s (float *X, const float *Xr, const float *Xz, const float *U, const float *Ur, const float *Uz, const int N, const int T, const int dim, const char iscolmajor);
int gru3_inplace_d (double *X, const double *Xr, const double *Xz, const double *U, const double *Ur, const double *Uz, const int N, const int T, const int dim, const char iscolmajor);
...
int gru3_s (float *Y, const float *X, const float *Xr, const float *Xz, const float *U, const float *Ur, const float *Uz, const int N, const int T, const int dim, const char iscolmajor)
{
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            ...
            {
                tN = t*N;
                cblas_scopy(N,&Xr[tN],1,R,1); cblas_scopy(N,&Xz[tN],1,Z,1);
        ...
        else
        {
            ...
            for (t=1; t<T; t++)
            {
                cblas_scopy(N,&Xr[t],T,R,1); cblas_scopy(N,&Xz[t],T,Z,1);
...
int gru3_d (double *Y, const double *X, const double *Xr, const double *Xz, const double *U, const double *Ur, const double *Uz, const int N, const int T, const int dim, const char iscolmajor)
{
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            ...
            {
                tN = t*N;
                cblas_dcopy(N,&Xr[tN],1,R,1); cblas_dcopy(N,&Xz[tN],1,Z,1);
        ...
        else
        {
            ...
            for (t=1; t<T; t++)
            {
                cblas_dcopy(N,&Xr[t],T,R,1); cblas_dcopy(N,&Xz[t],T,Z,1);
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            ...
            for (t=1; t<T; t++)
            {
                tN = t*N;
                cblas_scopy(N,&Xr[tN],1,R,1); cblas_scopy(N,&Xz[tN],1,Z,1);
                cblas_sgemv(CblasColMajor,CblasNoTrans,N,N,o,Ur,N,&X[tN-N],1,o,R,1);
                cblas_sgemv(CblasColMajor,CblasNoTrans,N,N,o,Uz,N,&X[tN-N],1,o,Z,1);
                for (n=0; n<N; n++)
                {
                    //R[n] = 1.0f / (1.0f+expf(-R[n]));
                    Z[n] = 1.0f / (1.0f+expf(-Z[n]));
        ...
        else
        {
            ...
            for (t=1; t<T; t++)
            {
                ...
                cblas_sgemv(CblasRowMajor,CblasNoTrans,N,N,o,Ur,N,&X[t-1],T,o,Z,1);
                for (n=0; n<N; n++)
                {
                    Z[n] = 1.0f / (1.0f+expf(-Z[n]));
    ...
    else if (dim==1)
    {
        if (iscolmajor)
        {
            ...
            for (t=1; t<T; t++)
            {
                ...
                cblas_sgemv(CblasColMajor,CblasTrans,N,N,o,Ur,N,&X[t-1],T,o,Z,1);
                for (n=0; n<N; n++)
                {
                    Z[n] = 1.0f / (1.0f+expf(-Z[n]));
                    H[n] = X[t-1+n*T] / (1.0f+expf(-R[n]));
        ...
        else
        {
            ...
            for (t=1; t<T; t++)
            {
                tN = t*N;
                cblas_scopy(N,&Xr[tN],1,R,1); cblas_scopy(N,&Xz[tN],1,Z,1);
                cblas_sgemv(CblasRowMajor,CblasTrans,N,N,o,Ur,N,&X[tN-N],1,o,R,1);
                cblas_sgemv(CblasRowMajor,CblasTrans,N,N,o,Uz,N,&X[tN-N],1,o,Z,1);
                for (n=0; n<N; n++)
                {
                    Z[n] = 1.0f / (1.0f+expf(-Z[n]));
                    H[n] = X[tN-N+n] / (1.0f+expf(-R[n]));
    ...
    else if (dim==0)
    {
        if (iscolmajor)
        {
            ...
            for (t=1; t<T; t++)
            {
                tN = t*N;
                cblas_dcopy(N,&Xr[tN],1,R,1); cblas_dcopy(N,&Xz[tN],1,Z,1);
                cblas_dgemv(CblasColMajor,CblasNoTrans,N,N,o,Ur,N,&X[tN-N],1,o,R,1);
                cblas_dgemv(CblasColMajor,CblasNoTrans,N,N,o,Uz,N,&X[tN-N],1,o,Z,1);
                for (n=0; n<N; n++)
                {
                    Z[n] = 1.0 / (1.0+exp(-Z[n]));
        ...
        else
        {
            ...
            for (t=1; t<T; t++)
            {
                ...
                cblas_dgemv(CblasRowMajor,CblasNoTrans,N,N,o,Ur,N,&X[t-1],T,o,Z,1);
                for (n=0; n<N; n++)
                {
                    Z[n] = 1.0 / (1.0+exp(-Z[n]));
    ...
    else if (dim==1)
    {
        if (iscolmajor)
        {
            ...
            for (t=1; t<T; t++)
            {
                ...
                cblas_dgemv(CblasColMajor,CblasTrans,N,N,o,Ur,N,&X[t-1],T,o,Z,1);
                for (n=0; n<N; n++)
                {
                    Z[n] = 1.0 / (1.0+exp(-Z[n]));
                    H[n] = X[t-1+n*T] / (1.0+exp(-R[n]));
        ...
        else
        {
            ...
            for (t=1; t<T; t++)
            {
                tN = t*N;
                cblas_dcopy(N,&Xr[tN],1,R,1); cblas_dcopy(N,&Xz[tN],1,Z,1);
                cblas_dgemv(CblasRowMajor,CblasTrans,N,N,o,Ur,N,&X[tN-N],1,o,R,1);
                cblas_dgemv(CblasRowMajor,CblasTrans,N,N,o,Uz,N,&X[tN-N],1,o,Z,1);
                for (n=0; n<N; n++)
                {
                    Z[n] = 1.0 / (1.0+exp(-Z[n]));
                    H[n] = X[tN-N+n] / (1.0+exp(-R[n]));

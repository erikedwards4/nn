...
int elu_s (float *Y, const float *X, const size_t N, const float alpha)
{
    ...
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = (*X<0.0f) ? expf(*X-1.0f) : *X; }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = (*X<0.0f) ? alpha*expf(*X-1.0f) : *X; }
    }
...
int elu_d (double *Y, const double *X, const size_t N, const double alpha)
{
    ...
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = (*X<0.0) ? exp(*X-1.0) : *X; }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = (*X<0.0) ? alpha*exp(*X-1.0) : *X; }
    }
...
int elu_inplace_s (float *X, const size_t N, const float alpha)
{
    ...
    {
        for (size_t n=0; n<N; ++n, ++X) { if (*X<0.0f) { *X = expf(*X-1.0f); } }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X) { if (*X<0.0f) { *X = alpha*expf(*X-1.0f); } }
    }
...
int elu_inplace_d (double *X, const size_t N, const double alpha)
{
    ...
    {
        for (size_t n=0; n<N; ++n, ++X) { if (*X<0.0) { *X = exp(*X-1.0); } }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X) { if (*X<0.0) { *X = alpha*exp(*X-1.0); } }
    }

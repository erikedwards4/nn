...
int logistic_s (float *Y, const float *X, const size_t N, const float alpha)
{
    ...
    if (alpha==1.0f)
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = 1.0f/(1.0f+expf(-*X)); }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = powf(1.0f+expf(-*X),-alpha); }
    }
...
int logistic_d (double *Y, const double *X, const size_t N, const double alpha)
{
    ...
    if (alpha==1.0)
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = 1.0/(1.0+exp(-*X)); }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X, ++Y) { *Y = pow(1.0+exp(-*X),-alpha); }
    }
...
int logistic_inplace_s (float *X, const size_t N, const float alpha)
{
    ...
    if (alpha==1.0f)
    {
        for (size_t n=0; n<N; ++n, ++X) { *X = 1.0f/(1.0f+expf(-*X)); }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X) { *X = powf(1.0f+expf(-*X),-alpha); }
    }
...
int logistic_inplace_d (double *X, const size_t N, const double alpha)
{
    ...
    if (alpha==1.0)
    {
        for (size_t n=0; n<N; ++n, ++X) { *X = 1.0/(1.0+exp(-*X)); }
    }
    else
    {
        for (size_t n=0; n<N; ++n, ++X) { *X = pow(1.0+exp(-*X),-alpha); }
    }

...
int selu_s (float *Y, const float *X, const size_t N)
{
    ...
    for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = (*X<0.0f) ? sc*(expf(*X)-1.0f) : lam**X; }
...
int selu_d (double *Y, const double *X, const size_t N)
{
    ...
    for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = (*X<0.0) ? sc*(exp(*X)-1.0) : lam**X; }
...
int selu_inplace_s (float *X, const size_t N)
{
    ...
    for (size_t n=N; n>0u; --n, ++X) { *X = (*X<0.0f) ? sc*(expf(*X)-1.0f) : lam**X; }
...
int selu_inplace_d (double *X, const size_t N)
{
    ...
    for (size_t n=N; n>0u; --n, ++X) { *X = (*X<0.0) ? sc*(exp(*X)-1.0) : lam**X; }
